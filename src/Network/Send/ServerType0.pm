...
sub sendAttack {
	...
	if ($self->{serverType} == 0) {
		...
		$msg = pack("C*", 0x90, 0x01) . junk(5) . $monID . junk(6) . pack("C", $flag);
...
#?
		$msg .= junk(16) . pack("C1", $serv) . junk(3);
	}
	$self->sendToServer($msg);
...
sub sendGetPlayerInfo {
	...
	if ($self->{serverType} == 0) {
		...
		$msg = pack("C*", 0x8c, 0x00) . junk(6) . $ID;
...
sub sendLook {
	...
	if ($self->{serverType} == 0) {
		...
		$msg = pack("C*", 0x85, 0x00) . junk(5) .
			pack("C*", $head, 0x00) . junk(2) .
			pack("C*", $body);
...
sub sendStorageAdd {
	...
	if ($self->{serverType} == 0) {
		...
		$msg = pack("C*", 0x94, 0x00) . junk(3) .
			pack("v*", $index) . junk(12) .
			pack("V*", $amount);
...
sub sendStand {
	...
	if ($self->{serverType} == 0) {
		...
		$msg = pack("C*", 0x90, 0x01) . junk(5) . junk(4) .junk(6) . pack("C", 0x03);
...
sub sendSync {
	...
	if ($self->{serverType} == 0) {
		...
		$msg .= pack("C*", 0x00, 0x00, 0x3F) if (!$initialSync);
		$msg .= pack("C*", 0x00, 0x00, 0x00, 0xA0);
		$msg .= $syncSync;
...
sub sendTake {
	...
	if ($self->{serverType} == 0) {
		...
		$msg = pack("C*", 0xF5, 0x00) . junk(7) . $itemID;
